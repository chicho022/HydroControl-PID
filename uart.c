...
static float sp_nivel = 20.0f;
static int modo = 0;

static float nivel = 10.0f;
static int u_pwm = 0;
/* USER CODE END PV */
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  // (prueba UART simple: sin interrupciones
  /* USER CODE END 2 */
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  // 2) ECO: si llega un byte desde ESP32, lo devuelvo
